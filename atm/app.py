...
class App(ctk.CTk):
    def __init__(self):
        ...
        # State management
        self.windowState = "INSERT_CARD"  # States: INSERT_CARD, ENTER_PIN, MENU, BALANCE, WITHDRAW, DEPOSIT, CONFIRM
    ...
    def update_screen(self):
        ...
        if self.windowState == "INSERT_CARD":
            ...
            new_balance = self.db.get_balance(self.current_account)
    ...
    def handle_enter(self):
        ...
        if self.windowState == "INSERT_CARD":
            if len(self.current_input) >= 4:  # Minimum account number length
                if self.db.account_exists(self.current_input):
            ...
            if len(self.current_input) == 4:
                if self.db.verify_pin(self.current_account, self.current_input):
                    self.current_input = ""
            ...
            if self.current_input:
                try:
                    ...
                    elif amount > self.db.get_balance(self.current_account):
    ...
    def handle_confirmation(self, response):
        ...
        if response == "y":  # Yes
            if self.windowState == "CONFIRM_WITHDRAW":
                success, result = self.db.withdraw(self.current_account, self.pending_amount)
                if success:
                ...
                success, result = self.db.deposit(self.current_account, self.pending_amount)
                if success:
