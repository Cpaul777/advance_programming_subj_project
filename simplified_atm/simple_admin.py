...
class SimpleAdminPanel(ctk.CTk):
    def __init__(self):
        super().__init__()
        
        # Window setup
        self.geometry("900x600")
        self.title("Simple ATM - Admin Panel")
    ...
    def create_operations_panel(self):
        ...
        ops_title.grid(row=0, column=0, pady=15, padx=10, sticky="ew")
        
        # Create scrollable frame for all operations
        ops_scroll = ctk.CTkScrollableFrame(ops_frame, fg_color="transparent")
        ops_scroll.grid(row=1, column=0, sticky="nsew", padx=5, pady=(0, 5))
        ops_scroll.grid_columnconfigure(0, weight=1)
        
        # Create Account Section
        create_frame = ctk.CTkFrame(ops_scroll, fg_color=LIGHT_COLOR, corner_radius=8)
        ...
        modify_frame = ctk.CTkFrame(ops_scroll, fg_color=LIGHT_COLOR, corner_radius=8)
        ...
        history_frame = ctk.CTkFrame(ops_scroll, fg_color=LIGHT_COLOR, corner_radius=8)
    ...
    def refresh_accounts(self):
        ...
        # Get all accounts
        accounts = self.data_store.get_all_accounts()
    ...
    def create_account(self):
        ...
        # Create account
        success = self.data_store.create_account(acc_num, pin, name, balance)
    ...
    def view_account(self):
        ...
        account = self.data_store.get_account_info(acc_num)
...
Created: {account['created_at']}
    ...
    def delete_account(self):
        ...
        if confirm:
            self.data_store.hard_delete_account(acc_num)
    ...
    def view_history(self):
        ...
        if not self.data_store.account_exists(acc_num):
            messagebox.showerror("Error", "Account not found!")
            return
        ...
        # Get transactions
        transactions = self.data_store.get_transaction_history(acc_num, limit=50)
